...
const UsersEdit = () => {
    ...
    const cancel = () => {
        var {name, nickname,password,level,active} = document.forms[0]; 
        var hasChanges = name.value.length > 0 ||  nickname.value.length > 0 || password.value.length > 0 || level.value.length > 0 || active.value.length > 0;
        if(hasChanges){
